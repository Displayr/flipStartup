...
for (by in c("week", "month", "quarter", "year"))
    test_that(paste("Creating RevenueData", by),
          {
            # Beginning and end
            expect_error(capture.output(rd <- RevenueData(d$AUD, d$ValidFrom, d$ValidTo, id = d$name, by = by, subset = d$validInvoice == 1)), NA)
            expect_error(capture.output(RevenueGrowthAccounting(rd)), NA)

            expect_error(capture.output(rd <- RevenueData(d$AUD, d$ValidFrom, d$ValidTo, start = start, id = d$name, by = by, subset = d$validInvoice == 1)), NA)
            expect_error(capture.output(RevenueGrowthAccounting(rd)), NA)
