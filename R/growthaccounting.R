...
#' @param x A RevenueGrowthAccounting object.
#' @param period A \code{character} indicating the period to plot (relative to the previous period). If NULL,
#' the total, aggregated across all periods, is shown.
#' @importFrom flipFormat FormatAsPercent
#' @export
Waterfall <- function(x, period = NULL)
{
    total <- is.null(period)
    y <- if (total) x$Growth else unlist(x$Table[period, ])
    ...
    if (!total)
    {
        period.names <- names(r)
        period.counter <- match(period, period.names)
    }
    base <- if(total) sum(r[-k]) else r[period.counter - 1]
    ...
    {
        if (is.null(period))
            period <- period.names[nrow(table)]
        title <- paste0(title, ": ", period, " relative to ", period.names[period.counter - 1], "")
    }
    # Result
...
plot.Waterfall <- function(x, ...)
{
    ...
    layout(p, barmode = "stack", showlegend = FALSE, annotations = a,
           ...
           yaxis = list(title = x$title, zeroline = TRUE, showticklabels = FALSE, showgrid = FALSE))
}
